...
docker-compose run --rm -T agent curl -X POST --data @/tmp/input.json -H "Content-Type:application/yang-data+json" --user my-serial-number:my-secret "${BOOTSTRAP}" | tee /tmp/post_rpc_input.json
